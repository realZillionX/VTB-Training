...
def main():
    ...
    for i, sample in enumerate(samples):
        ...
        print(f"Prompt: {sample['query']}")
        
        # Prepare inputs
        # Swift inference expects: inputs (str), history (list), images (list)
        images = sample.get('images', [])
        
        # Run inference
        # inference(model, template, query, history=None, images=None, **kwargs)
        # Note: image handling might vary by swift version, passing list of paths usually works
        
        response, _ = inference(model, template, sample['query'], images=images)
